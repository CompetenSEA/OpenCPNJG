...
#if 1  // wxUSE_FONTDLG && (defined(__WXGTK__) || defined(__WXGPE__) ||
...
class WXDLLEXPORT OCPNFontPreviewer : public wxWindow {
public:
  OCPNFontPreviewer(wxWindow* parent, const wxSize& sz = wxDefaultSize)
...
wxBEGIN_EVENT_TABLE(OCPNFontPreviewer, wxWindow)
    EVT_PAINT(OCPNFontPreviewer::OnPaint) wxEND_EVENT_TABLE()

        void OCPNFontPreviewer::OnPaint(wxPaintEvent& WXUNUSED(event)) {
  wxPaintDC dc(this);
...
wxBEGIN_EVENT_TABLE(ocpnGenericFontDialog, wxDialog)
    EVT_CHECKBOX(wxID_FONT_UNDERLINE, ocpnGenericFontDialog::OnChangeFont)
        EVT_CHOICE(wxID_FONT_STYLE, ocpnGenericFontDialog::OnChangeFont)
            EVT_CHOICE(wxID_FONT_WEIGHT, ocpnGenericFontDialog::OnChangeFont)
                EVT_CHOICE(wxID_FONT_FAMILY,
                           ocpnGenericFontDialog::OnChangeFont)
                    EVT_CHOICE(wxID_FONT_COLOUR,
                               ocpnGenericFontDialog::OnChangeFont)
#if USE_SPINCTRL_FOR_POINT_SIZE
                        EVT_SPINCTRL(wxID_FONT_SIZE,
                                     ocpnGenericFontDialog::OnChangeSize)
                            EVT_TEXT(wxID_FONT_SIZE,
                                     ocpnGenericFontDialog::OnChangeFont)
#else
                        EVT_CHOICE(wxID_FONT_SIZE,
                                   ocpnGenericFontDialog::OnChangeFont)
#endif
                                EVT_CLOSE(ocpnGenericFontDialog::OnCloseWindow)
                                    wxEND_EVENT_TABLE()
...
void ocpnGenericFontDialog::Init() {
  m_useEvents = false;
  m_previewer = NULL;
  Create(m_parent);
}

ocpnGenericFontDialog::~ocpnGenericFontDialog() {}

void ocpnGenericFontDialog::OnCloseWindow(wxCloseEvent& WXUNUSED(event)) {
...
int ocpnGenericFontDialog::ShowModal() {
...
void ocpnGenericFontDialog::CreateWidgets() {
  wxString *families = new wxString[6], *styles = new wxString[3],
  ...
  m_familyChoice = new wxChoice(this, wxID_FONT_FAMILY, wxDefaultPosition,
                                wxDefaultSize, 5, families, 0);
  m_familyChoice->SetHelpText(_("The font family."));
  if (ShowToolTips()) m_familyChoice->SetToolTip(_("The font family."));
  ...
  m_styleChoice = new wxChoice(this, wxID_FONT_STYLE, wxDefaultPosition,
                               wxDefaultSize, 3, styles, 0);
  m_styleChoice->SetHelpText(_("The font style."));
  if (ShowToolTips()) m_styleChoice->SetToolTip(_("The font style."));
  ...
  m_weightChoice = new wxChoice(this, wxID_FONT_WEIGHT, wxDefaultPosition,
                                wxDefaultSize, 3, weights, 0);
  m_weightChoice->SetHelpText(_("The font weight."));
  if (ShowToolTips()) m_weightChoice->SetToolTip(_("The font weight."));
  ...
  if (m_fontData.GetEnableEffects()) {
    ...
    m_colourChoice =
        new wxChoice(this, wxID_FONT_COLOUR, wxDefaultPosition, colourSize,
                     NUM_COLS, ocpnColourDialogNames, 0);
    m_colourChoice->SetHelpText(_("The font colour."));
    if (ShowToolTips()) m_colourChoice->SetToolTip(_("The font colour."));
...
#else
  m_pointSizeChoice = new wxChoice(this, wxID_FONT_SIZE, wxDefaultPosition,
                                   wxDefaultSize, 40, pointSizes, 0);
  m_pointSizeChoice->SetHelpText(_("The font point size."));
  if (ShowToolTips()) m_pointSizeChoice->SetToolTip(_("The font point size."));
  ...
  m_previewer = new OCPNFontPreviewer(this);
  m_previewer->SetHelpText(_("Shows the font preview."));
  if (ShowToolTips()) m_previewer->SetToolTip(_("Shows the font preview."));
  ...
  m_familyChoice->SetStringSelection(
      ocpnFontFamilyIntToString(m_dialogFont.GetFamily()));
  m_styleChoice->SetStringSelection(
      ocpnFontStyleIntToString(m_dialogFont.GetStyle()));
  m_weightChoice->SetStringSelection(
      ocpnFontWeightIntToString(m_dialogFont.GetWeight()));
  ...
  GetSizer()->SetItemMinSize(m_previewer, is_pda ? 100 : 430,
                             is_pda ? 40 : 100);
...
void ocpnGenericFontDialog::InitializeFont() {
  ...
  if (m_previewer) m_previewer->SetFont(m_dialogFont);
}

void ocpnGenericFontDialog::OnChangeFont(wxCommandEvent& WXUNUSED(event)) {
  DoChangeFont();
}

void ocpnGenericFontDialog::DoChangeFont() {
  if (!m_useEvents) return;

  wxFontFamily fontFamily =
      ocpnFontFamilyStringToInt(m_familyChoice->GetStringSelection());
  wxFontWeight fontWeight =
      ocpnFontWeightStringToInt(m_weightChoice->GetStringSelection());
  wxFontStyle fontStyle =
      ocpnFontStyleStringToInt(m_styleChoice->GetStringSelection());
#if USE_SPINCTRL_FOR_POINT_SIZE
  ...
  // can't edit it m_dialogFont is always initialized because of the call to
  // InitializeFont
  ...
  m_dialogFont =
      wxFont(fontSize, fontFamily, fontStyle, fontWeight, (fontUnderline != 0));
  m_previewer->SetFont(m_dialogFont);

  if (m_colourChoice) {
    if (!m_colourChoice->GetStringSelection().empty()) {
      wxColour col =
          wxTheColourDatabase->Find(m_colourChoice->GetStringSelection());
      if (col.IsOk()) {
...
#if USE_SPINCTRL_FOR_POINT_SIZE
void ocpnGenericFontDialog::OnChangeSize(wxSpinEvent& WXUNUSED(event)) {
  DoChangeFont();
}
#endif
